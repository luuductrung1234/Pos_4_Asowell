...
namespace POS.Mapping
{
    ...
    public partial class OrderNoteMapping : System.Data.Entity.ModelConfiguration.EntityTypeConfiguration<OrderNote>
    {
        ...
        public OrderNoteMapping(string schema)
        {
            ...
            Property(x => x.Ordertime).HasColumnName(@"ordertime").HasColumnType("date").IsRequired();
            ...
            // Foreign keys
            HasOptional(a => a.Customer).WithMany(b => b.OrderNotes).HasForeignKey(c => c.CusId).WillCascadeOnDelete(false); // fk_customerowner
            HasOptional(a => a.Employee).WithMany(b => b.OrderNotes).HasForeignKey(c => c.EmpId).WillCascadeOnDelete(false); // fk_employeerespond
            InitializePartial();
        }
